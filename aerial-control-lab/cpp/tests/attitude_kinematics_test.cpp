...
TEST(AttitudeKinematicsTest, ZeroAngularRateGivesZeroDerivative) {
    ...
    const StateVector qDot = attitudeKinematicsDerivative(
        0.0, q, Eigen::Vector3d::Zero());
...
TEST(AttitudeKinematicsTest, IdentityAttitudeTracksAngularRateSign) {
    ...
    const StateVector positive = attitudeKinematicsDerivative(
        0.0, q, Eigen::Vector3d{0.0, 0.0, 2.0});
    const StateVector negative = attitudeKinematicsDerivative(
        0.0, q, Eigen::Vector3d{0.0, 0.0, -2.0});
...
TEST(AttitudeKinematicsTest, NonidentityAttitudePreservesBodyRateOrder) {
    ...
    const StateVector qDot = attitudeKinematicsDerivative(
        0.0, q, Eigen::Vector3d{2.0, 0.0, 0.0});
...
TEST(AttitudeKinematicsTest, NonunitIntermediateStateIsNotNormalized) {
    ...
    const StateVector qDot = attitudeKinematicsDerivative(
        0.0, q, Eigen::Vector3d{1.0, 2.0, 3.0});
...
TEST(AttitudeKinematicsTest, RejectsInvalidInputs) {
    ...
    EXPECT_THROW(
        attitudeKinematicsDerivative(0.0, StateVector::Zero(3), zeroRate),
        std::invalid_argument);
    EXPECT_THROW(
        attitudeKinematicsDerivative(0.0, StateVector::Zero(4), zeroRate),
        std::invalid_argument);

    q(1) = std::numeric_limits<double>::quiet_NaN();
    EXPECT_THROW(
        attitudeKinematicsDerivative(0.0, q, zeroRate),
        std::invalid_argument);
    q(1) = 0.0;

    EXPECT_THROW(
        attitudeKinematicsDerivative(
            0.0,
            q,
            Eigen::Vector3d{
    ...
    EXPECT_THROW(
        attitudeKinematicsDerivative(
            std::numeric_limits<double>::quiet_NaN(), q, zeroRate),
        std::invalid_argument);
}
