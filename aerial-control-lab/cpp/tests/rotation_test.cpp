...
namespace
{
  ...
  Eigen::Quaterniond axisAngleQuaternion(const Eigen::Vector3d &axis, const double angle_rad) { return Eigen::Quaterniond{Eigen::AngleAxisd{angle_rad, axis.normalized()}}; }
  ...
  TEST(RotationUtilitiesTest, IdentityRotationMatchesPythonFixedVector)
  {
    ...
    expectVectorNear(aerial_control::rotateBodyToNed(identity, vector_body), vector_body);
    expectVectorNear(aerial_control::rotateNedToBody(identity, vector_body), vector_body);
  }
  ...
  TEST(RotationUtilitiesTest, QuarterTurnAboutDownAxisRotatesNorthToEast)
  {
    ...
    const Eigen::Quaterniond q_nb{half_sqrt_two, 0.0, 0.0, half_sqrt_two};

    // 绕 NED 向下轴正转 90° 后，机体前向（北）应转到东向。
    expectVectorNear(aerial_control::rotateBodyToNed(q_nb, Eigen::Vector3d{1.0, 0.0, 0.0}), Eigen::Vector3d{0.0, 1.0, 0.0});
  }

  TEST(RotationUtilitiesTest, CompositionAppliesFirstRotationThenSecond)
  {
    // 固定顺序：先绕 x 轴正转 90°，再绕 z 轴正转 90°。
    const Eigen::Quaterniond q_x = axisAngleQuaternion(Eigen::Vector3d::UnitX(), std::acos(-1.0) / 2.0);
    const Eigen::Quaterniond q_z = axisAngleQuaternion(Eigen::Vector3d::UnitZ(), std::acos(-1.0) / 2.0);
    const Eigen::Vector3d vector_body{0.0, 1.0, 0.0};

    // sequential 显式执行两次旋转，作为组合结果的独立对照。
    const Eigen::Vector3d sequential = aerial_control::rotateBodyToNed(q_z, aerial_control::rotateBodyToNed(q_x, vector_body));
    const Eigen::Quaterniond composed = aerial_control::composeRotation(q_x, q_z);
    const Eigen::Vector3d composed_result = aerial_control::rotateBodyToNed(composed, vector_body);
  ...
  TEST(RotationUtilitiesTest, BodyAndNedTransformsAreMutualInverses)
  {
    ...
    const Eigen::Quaterniond q_nb = axisAngleQuaternion(Eigen::Vector3d{0.3, 0.7, -0.2}, -0.83);
    const Eigen::Vector3d vector_body{-1.0, 2.5, 0.75};

    const Eigen::Vector3d vector_ned = aerial_control::rotateBodyToNed(q_nb, vector_body);
    const Eigen::Vector3d recovered_body = aerial_control::rotateNedToBody(q_nb, vector_ned);
  ...
  TEST(RotationUtilitiesTest, NormalizationUsesScalarFirstConstructorOrder)
  {
    ...
    const Eigen::Quaterniond normalized = aerial_control::normalizeQuaternion(input);
  ...
  TEST(RotationUtilitiesTest, QuaternionAndNegationRotateIdentically)
  {
    ...
    const Eigen::Quaterniond q_nb = axisAngleQuaternion(Eigen::Vector3d::UnitY(), 0.61);
    const Eigen::Quaterniond negative_q_nb{-q_nb.w(), -q_nb.x(), -q_nb.y(), -q_nb.z()};
    const Eigen::Vector3d vector_body{1.0, 2.0, 3.0};

    expectVectorNear(aerial_control::rotateBodyToNed(q_nb, vector_body), aerial_control::rotateBodyToNed(negative_q_nb, vector_body));
  }
  ...
  TEST(RotationUtilitiesTest, RejectsInvalidQuaternionAndVectorInputs)
  {
    ...
    const Eigen::Quaterniond non_finite{1.0, 0.0, std::numeric_limits<double>::infinity(), 0.0};
    const Eigen::Vector3d invalid_vector{1.0, std::numeric_limits<double>::quiet_NaN(), 3.0};

    EXPECT_THROW(aerial_control::normalizeQuaternion(zero), std::invalid_argument);
    EXPECT_THROW(aerial_control::normalizeQuaternion(non_finite), std::invalid_argument);
    EXPECT_THROW(aerial_control::rotateBodyToNed(Eigen::Quaterniond::Identity(), invalid_vector), std::invalid_argument);
  }
