...
namespace aerial_control
{
  ...
  StateVector pointMassDerivative(double t, const StateVector &state, const PointMassParameters &parameters, const PointMassInput &input)
  {
    ...
    {
      throw std::invalid_argument("State vector must have finite values.");
    }

    if (parameters.massKg <= 0.0 || std::isfinite(parameters.massKg) == false)
    {
      throw std::invalid_argument("Mass must be positive and finite.");
    }

    if (input.totalThrustN < 0.0 || std::isfinite(input.totalThrustN) == false)
    {
      throw std::invalid_argument("Total thrust must be non-negative and finite.");
    }
    ...
    const Eigen::Vector3d thrust_ned = rotateBodyToNed(input.qNb, thrust_frd);
    ...
    const Eigen::Vector3d acceleration_ned = thrust_ned / parameters.massKg + gravity_ned;
