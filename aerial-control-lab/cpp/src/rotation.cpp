...
namespace aerial_control
{
  namespace
  {
    ...
    Eigen::Quaterniond normalizeChecked(const Eigen::Quaterniond &quaternion, const char *argument_name)
    {
      ...
      {
        throw std::invalid_argument(std::string{argument_name} + " must have a non-zero norm to represent a rotation");
      }
    ...
    void validateVector(const Eigen::Vector3d &vector, const char *argument_name)
    {
      if (!vector.allFinite())
      {
        throw std::invalid_argument(std::string{argument_name} + " must contain only finite values");
  ...
  Eigen::Quaterniond normalizeQuaternion(const Eigen::Quaterniond &quaternion)
  {
    // 对外接口复用内部检查函数，并用稳定参数名生成异常信息。
    return normalizeChecked(quaternion, "quaternion");
  }

  Eigen::Quaterniond composeRotation(const Eigen::Quaterniond &first_rotation, const Eigen::Quaterniond &second_rotation)
  {
    // 分别归一化两个输入，避免输入范数漂移在乘法中引入缩放误差。
    const Eigen::Quaterniond first = normalizeChecked(first_rotation, "first_rotation");
    const Eigen::Quaterniond second = normalizeChecked(second_rotation, "second_rotation");
  ...
  Eigen::Vector3d rotateBodyToNed(const Eigen::Quaterniond &q_nb, const Eigen::Vector3d &vector_body)
  {
  ...
  Eigen::Vector3d rotateNedToBody(const Eigen::Quaterniond &q_nb, const Eigen::Vector3d &vector_ned)
  {
