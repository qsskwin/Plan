...
double referenceHeightAtTime(const CaseDefinition& caseDefinition,
                             const double timeS) {
  return timeS < caseDefinition.stepTimeS
             ? caseDefinition.initialReferenceHeightM
             : caseDefinition.stepReferenceHeightM;
}

CaseResult runCase(const CaseDefinition& caseDefinition,
  ...
  result.samples.reserve(caseDefinition.numIntegrationSteps /
                         caseDefinition.controlStride);
  ...
  const aerial_control::DerivativeFunction dynamics =
      [&plantParameters, &heldInput](
        ...
        return aerial_control::pointMassDerivative(timeS, evaluationState,
                                                   plantParameters, heldInput);
      };

  for (std::size_t step = 0U; step < caseDefinition.numIntegrationSteps;
       ++step) {
    ...
    result.finalState = aerial_control::rk4Step(
        dynamics, timeS, result.finalState, caseDefinition.dtS);
...
CaseSummary summarizeCase(const CaseDefinition& caseDefinition,
                          const CaseResult& result) {
  const double finalTimeS =
      static_cast<double>(caseDefinition.numIntegrationSteps) *
      caseDefinition.dtS;
  ...
  return {
      ...
      -result.finalState(5),
      lastSecondSampleCount,
      lastSecondHeightErrorSumM / static_cast<double>(lastSecondSampleCount),
  };
}

void printSummary(const CaseDefinition& caseDefinition,
                  const CaseResult& result, const CaseSummary& summary) {
  ...
  std::cout << "final vertical speed: " << summary.finalVerticalSpeedMps
            << " m/s\n";
  ...
  std::cout << "last-second sample count: " << summary.lastSecondSampleCount
            << '\n';
  std::cout << "last-second average height error: "
            << summary.lastSecondAverageHeightErrorM << " m\n";
}

void printSaturationChecks(
    const aerial_control::AltitudePdController& controller) {
  const aerial_control::AltitudeCommand upperLimitCommand =
      controller.compute(10.0, 0.0, 0.0);
  const aerial_control::AltitudeCommand lowerLimitCommand =
      controller.compute(0.0, 10.0, 0.0);

  std::cout << "upper-limit check: raw_thrust=" << upperLimitCommand.rawThrustN
            << " N, applied_thrust=" << upperLimitCommand.appliedThrustN
            << " N\n";
  std::cout << "lower-limit check: raw_thrust=" << lowerLimitCommand.rawThrustN
            << " N, applied_thrust=" << lowerLimitCommand.appliedThrustN
            << " N\n";
}

void exportSamplesCsv(const std::string& outputPath,
  ...
  for (const Sample& sample : samples) {
    output << sample.timeS << ',' << sample.referenceHeightM << ','
           << sample.heightM << ',' << sample.verticalSpeedMps << ','
           << sample.rawThrustN << ',' << sample.appliedThrustN << '\n';
  }
}
