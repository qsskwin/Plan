...
namespace week1_wed_task_a
{
  ...
  LifecycleProbe::LifecycleProbe(LifecycleStats &stats, const int value) noexcept : stats_(stats), value_(value)
  {
  ...
  LifecycleProbe::LifecycleProbe(const LifecycleProbe &other) noexcept : stats_(other.stats_), value_(other.value_)
  {
  ...
  LifecycleProbe::LifecycleProbe(LifecycleProbe &&other) noexcept : stats_(other.stats_), value_(other.value_)
  {
  ...
  ScopeTimer::ScopeTimer(std::string label, std::ostream &output) : label_(std::move(label)), output_(&output), start_(Clock::now())
  {
  ...
  ScopeTimer::~ScopeTimer() noexcept
  {
    ...
    const auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(Clock::now() - start_);
