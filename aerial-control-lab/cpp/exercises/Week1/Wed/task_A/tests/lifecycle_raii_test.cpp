...
int main()
{
  ...
  LifecycleStats lifecycle_stats;
  {
    ...
    expect(lifecycle_stats.direct_constructions == 1, "直接构造应被记录一次");

    // 从左值 original 拷贝构造 copied：应记录一次拷贝，并完整保留整数值 42。
    LifecycleProbe copied(original);
    expect(lifecycle_stats.copy_constructions == 1, "拷贝构造应被记录一次");
    ...
    expect(lifecycle_stats.move_constructions == 1, "移动构造应被记录一次");
    expect(moved.value() == 42, "移动目标应接收原值");
    expect(original.value() == 0, "本练习定义的移动源应进入可观察的空状态");

    // 三个对象目前都还在内层作用域中，所以任何析构计数都意味着生命周期实现有误。
    expect(lifecycle_stats.destructions == 0, "离开作用域前不应析构三个局部对象");

    // 结束作用域时 moved、copied、original 会按构造的相反顺序自动析构。
  }
  expect(lifecycle_stats.destructions == 3, "离开作用域时三个对象都应被析构");
  ...
  expect(owner && owner->value() == 9, "工厂函数应返回拥有有效对象的 unique_ptr");
  ...
  expect(!owner && static_cast<bool>(new_owner), "移动 unique_ptr 后唯一所有权应转移到新所有者");

  // reset() 主动释放动态对象。LifecycleProbe 的析构函数应随之立刻执行一次。
  new_owner.reset();
  expect(ownership_stats.destructions == 1, "unique_ptr 释放所有权时应自动析构对象");
  ...
  expect(timer_message.find("[ScopeTimer] test scope:") != std::string::npos, "ScopeTimer 析构时应输出标签");
  expect(timer_message.find(" us") != std::string::npos, "ScopeTimer 应以微秒输出持续时间");
