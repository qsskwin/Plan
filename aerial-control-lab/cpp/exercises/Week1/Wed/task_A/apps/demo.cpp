...
namespace
{
  ...
  void print_stats(const std::string_view stage, const week1_wed_task_a::LifecycleStats &stats)
  {
    // 一次输出四类计数，便于对照下一条构造、移动或析构语句前后的变化。
    std::cout << stage << ": direct=" << stats.direct_constructions << ", copy=" << stats.copy_constructions << ", move=" << stats.move_constructions << ", destroyed=" << stats.destructions << '\n';
  }
...
int main()
{
  ...
  LifecycleStats lifecycle_stats;
  {
    ...
    std::cout << "copied value=" << copied.value() << ", moved value=" << moved.value() << '\n';
  ...
  std::cout << "ownership transferred=" << std::boolalpha << (!owner && static_cast<bool>(new_owner)) << '\n';
