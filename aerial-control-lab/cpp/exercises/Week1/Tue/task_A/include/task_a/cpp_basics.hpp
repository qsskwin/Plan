...
namespace task_a
{
  ...
  struct CopyTracked
  {
    ...
    CopyTracked(const CopyTracked &other) noexcept : copy_count(other.copy_count + 1) {}
  ...
  std::unordered_map<std::string, std::size_t> count_strings(const std::vector<std::string> &words);
