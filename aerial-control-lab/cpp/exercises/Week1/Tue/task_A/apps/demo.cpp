...
int main()
{
  ...
  std::cout << "copies (by value)=" << task_a::copies_seen_by_value(tracked) << ", copies (by const reference)=" << task_a::copies_seen_by_const_reference(tracked) << '\n';
  ...
  std::cout << std::boolalpha << "pointer incremented=" << incremented << ", value=" << pointed_value << ", null safely rejected=" << null_rejected << '\n';
  ...
  std::cout << "scaled Vec3=[" << doubled[0] << ", " << doubled[1] << ", " << doubled[2] << "]\n";
  ...
  std::cout << "word counts: ned=" << counts.at("ned") << ", frd=" << counts.at("frd") << '\n';
  ...
  std::cout << "unique_ptr consumed value=" << consumed.value() << ", caller still owns it=" << static_cast<bool>(owned) << '\n';
