...
namespace
{
  ...
  void expect_near(const double actual, const double expected, const std::string &message) { expect(std::abs(actual - expected) < 1e-12, message); }
...
int main()
{
  ...
  expect(task_a::copies_seen_by_const_reference(tracked) == 0, "const 引用传递不应复制对象");
  ...
  expect(counts.at("north") == 2 && counts.at("east") == 1 && counts.at("down") == 1, "unordered_map 应正确统计字符串频次");
