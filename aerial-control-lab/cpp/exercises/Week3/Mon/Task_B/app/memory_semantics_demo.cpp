...
{
  std::cout << "inside, before modification: " << value << '\n';

  // 修改的是局部形参副本。
  value = 99;

  std::cout << "inside, after modification: " << value << '\n';
}
...
int main()
{
  int value{10};
  int ref_value{10};
  std::cout << "caller, before function: " << value << '\n';

  // 值传递不会改变调用者的 value。
  modify_by_value(value);

  std::cout << "caller, after function: " << value << '\n';

  // 引用传递会改变调用者的 ref_value。
  std::cout << "caller, before function: " << ref_value << '\n';

  modify_by_reference(ref_value);

  std::cout << "caller, after function: " << ref_value << '\n';
  ...
  std::cout << "caller, after function: " << pointer_value << '\n';

  std::cout << "Status message: " << make_status_message() << '\n';
