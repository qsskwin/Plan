...
TEST(VectorInvalidationTest, EraseKeepsOnlyEarlierIteratorsValid)
{
  ...
  const auto old_capacity = values.capacity();
  const auto next = values.erase(values.begin() + 1); // 删除 20 删除位置及其后的旧迭代器均失效
  EXPECT_EQ(*first, 10);
  EXPECT_EQ(*next, 30);
...
TEST(UnorderedMapInvalidationTest, RehashInvalidatesIteratorsButKeepsElementPointersValid)
{
