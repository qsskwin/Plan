...
{
  ThreeAxisState make_three_axis_state(double x, double y, double z) { return {x, y, z}; }
  ...
  void push_rolling_sample(std::deque<double> &buffer, std::size_t max_capacity, double sample)
  {
  ...
  std::vector<double> largest_errors(const std::vector<double> &errors, std::size_t count)
  {
    if (count > errors.size())
    {
      throw std::invalid_argument("count must not be greater than the number of errors");
    }
