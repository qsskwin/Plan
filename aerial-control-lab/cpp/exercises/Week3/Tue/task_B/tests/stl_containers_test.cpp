...
{
  const auto state = aerial_control::week3_tue::make_three_axis_state(1.0, 2.0, 3.0);
...
TEST(PriorityQueueTest, ReturnsLargestErrorsInDescendingOrder)
{
  ...
  const auto largest_errors = aerial_control::week3_tue::largest_errors(errors, count);
...
TEST(PriorityQueueTest, RejectsCountGreaterThanErrorCount)
{
  ...
  EXPECT_THROW(aerial_control::week3_tue::largest_errors(errors, count), std::invalid_argument);
}
