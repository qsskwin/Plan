...
TEST(FreeFallDerivativeTest, ReturnsVelocityAndConstantGravity)
{
  ...
  const week2::fri::State derivative = week2::fri::free_fall_derivative(7.0, state);
...
TEST(EulerStepTest, AdvancesOneStepFromRest)
{
  ...
  const week2::fri::State initial_state = week2::fri::State::Zero();

  const week2::fri::State next_state = week2::fri::euler_step(0.0, initial_state, dt);
...
TEST(Rk4StepTest, MatchesAnalyticalSolutionForOneStep)
{
  ...
  const week2::fri::State next_state = week2::fri::rk4_step(0.0, initial_state, dt);

  const double expected_position = 0.5 * week2::fri::kGravity * dt * dt;
  const double expected_velocity = week2::fri::kGravity * dt;
...
TEST(FreeFallSimulationTest, BothMethodsMatchFinalVelocityAtTwoSeconds)
{
  ...
  const double expected_velocity = week2::fri::kGravity * end_time;

  EXPECT_NEAR(euler_state(1), expected_velocity, 1e-12);
  EXPECT_NEAR(rk4_state(1), expected_velocity, 1e-12);
}

TEST(IntegratorAccuracyTest, Rk4PositionErrorIsSmallerThanEulerAtTwoSeconds)
{
  constexpr double dt = 0.1;
  constexpr double end_time = 2.0;
  constexpr int number_of_steps = 20;
  ...
  const double expected_position = 0.5 * week2::fri::kGravity * end_time * end_time;

  const double euler_position_error = std::abs(euler_state(0) - expected_position);

  const double rk4_position_error = std::abs(rk4_state(0) - expected_position);
...
{
  const week2::fri::State state = week2::fri::State::Zero();

  EXPECT_THROW(week2::fri::euler_step(0.0, state, 0.0), std::invalid_argument);

  EXPECT_THROW(week2::fri::euler_step(0.0, state, -0.1), std::invalid_argument);

  EXPECT_THROW(week2::fri::rk4_step(0.0, state, 0.0), std::invalid_argument);

  EXPECT_THROW(week2::fri::rk4_step(0.0, state, -0.1), std::invalid_argument);
}
