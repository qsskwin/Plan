...
namespace
{
  ...
  std::vector<CaseData> buildCases()
  {
    ...
    const double hover_thrust_n = kMassKg * aerial_control::kStandardGravityMps2;
  ...
  {
    return {
        "mass_bias", "Level attitude with 10% mass increase", kMassBiasKg, kMassKg * aerial_control::kStandardGravityMps2, Eigen::Quaterniond{1.0, 0.0, 0.0, 0.0}, kT0S, 2.0, kDtS,
    };
  }

  aerial_control::StateVector analyticalState(const CaseData &case_data, const double time_s)
  {
    aerial_control::StateVector state = aerial_control::StateVector::Zero(6);
    const double elapsed_s = time_s - case_data.t0S;

    if (case_data.slug == "free_fall")
    {
      state(2) = 0.5 * aerial_control::kStandardGravityMps2 * elapsed_s * elapsed_s;
    ...
    {
      const double east_acceleration_mps2 = aerial_control::kStandardGravityMps2 * std::tan(kRollRad);
    ...
    {
      const double down_acceleration_mps2 = aerial_control::kStandardGravityMps2 - case_data.totalThrustN / case_data.massKg;
      state(2) = 0.5 * down_acceleration_mps2 * elapsed_s * elapsed_s;
      state(5) = down_acceleration_mps2 * elapsed_s;
    }
    else
    {
      throw std::invalid_argument("Analytical solution is not implemented for " + case_data.slug);
    }
  ...
  CaseResult runCase(const CaseData &case_data)
  {
    ...
    if (std::abs(static_cast<double>(num_steps) * case_data.dtS - duration_s) > 1.0e-12)
    {
    ...
    const aerial_control::DerivativeFunction dynamics = [&parameters, &input](const double time_s, const aerial_control::StateVector &state) { return aerial_control::pointMassDerivative(time_s, state, parameters, input); };
    ...
    result.numericalStates.push_back(aerial_control::StateVector::Zero(6));

    for (std::size_t step = 0; step < num_steps; ++step)
    {
      const double time_s = case_data.t0S + static_cast<double>(step) * case_data.dtS;
      result.numericalStates.push_back(aerial_control::rk4Step(dynamics, time_s, result.numericalStates.back(), case_data.dtS));

      double next_time_s = case_data.t0S + static_cast<double>(step + 1U) * case_data.dtS;
    ...
    {
      const aerial_control::StateVector analytical = analyticalState(case_data, result.times[index]);
      const aerial_control::StateVector absolute_error = (result.numericalStates[index] - analytical).cwiseAbs();
      result.maxPositionErrorM = std::max(result.maxPositionErrorM, absolute_error.head<3>().maxCoeff());
      result.maxVelocityErrorMps = std::max(result.maxVelocityErrorMps, absolute_error.tail<3>().maxCoeff());
      result.analyticalStates.push_back(analytical);
    }

    if (result.maxPositionErrorM > kPositionAtolM)
    {
      throw std::runtime_error(case_data.slug + " exceeded the position tolerance");
    }
    if (result.maxVelocityErrorMps > kVelocityAtolMps)
    {
      throw std::runtime_error(case_data.slug + " exceeded the velocity tolerance");
    }
    return result;
  }

  std::filesystem::path saveTrajectory(const std::filesystem::path &output_dir, const CaseData &case_data, const CaseResult &result)
  {
    std::filesystem::create_directories(output_dir);
    const std::filesystem::path output_path = output_dir / ("cpp_" + case_data.slug + ".csv");
    ...
    for (std::size_t index = 0; index < result.times.size(); ++index)
    {
      ...
      for (Eigen::Index component = 0; component < numerical.size(); ++component)
      {
        output << ',' << numerical(component);
      }
      for (Eigen::Index component = 0; component < analytical.size(); ++component)
      {
  ...
  void printSummary(const CaseData &case_data, const CaseResult &result)
  {
    ...
    std::cout << "Parameters: mass=" << case_data.massKg << " kg, thrust=" << case_data.totalThrustN << " N, q_nb=[" << case_data.qNb.w() << ", " << case_data.qNb.x() << ", " << case_data.qNb.y() << ", " << case_data.qNb.z() << "], dt=" << case_data.dtS << " s\n";
    ...
    std::cout << '\n';
    std::cout << "Maximum position error: " << result.maxPositionErrorM << " m\n";
    std::cout << "Maximum velocity error: " << result.maxVelocityErrorMps << " m/s\n\n";
  }
...
int main(int argc, char *argv[])
{
  try
  {
    std::filesystem::path output_dir = "python/exercises/Week4/Fri/task_A/outputs";
    ...
    for (int index = 1; index < argc; ++index)
    {
      ...
      else if (argument == "--output-dir")
      {
        if (index + 1 >= argc)
        {
          throw std::invalid_argument("--output-dir requires a path argument");
        }
    ...
    {
      throw std::invalid_argument("--include-mass-bias and --mass-bias-only are mutually exclusive");
    }
    ...
    std::vector<CaseData> cases = mass_bias_only ? std::vector<CaseData>{} : buildCases();
    if (include_mass_bias || mass_bias_only)
    {
    ...
    for (const CaseData &case_data : cases)
    {
      ...
      const std::filesystem::path output_path = saveTrajectory(output_dir, case_data, result);
