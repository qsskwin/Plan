...
aerial_control::StateVector simulateOscillator(double dt)
{
  ...
  const int stepCount = static_cast<int>(std::round(finalTime / dt));

  for (int step = 0; step < stepCount; ++step)
  {
    const double t = static_cast<double>(step) * dt;

    state = aerial_control::rk4Step(oscillatorDerivative, t, state, dt);
  }
...
{
  const aerial_control::StateVector numerical = simulateOscillator(dt);
