...
mass_kg = 1.5
phi = np.radians(10)
q_nb = [np.cos(phi/2), np.sin(phi/2), 0, 0]
...
def positive_roll_trajectory_dynamics(t, state):
...
times, states = simulate_fixed_step(
    positive_roll_trajectory_dynamics,
    initial_state,
    t0,
    tf,
    dt,
...
assert times.shape == (201,)
assert states.shape == (201, 6)
assert times[0] == t0
assert times[-1] == tf
...
analytical_position[:, 1] = 0.5 * total_thrust_n * np.sin(phi) * times**2 / mass_kg
analytical_velocity[:, 1] = total_thrust_n * np.sin(phi) * times / mass_kg
