...
- 首次正滚转测试被正式 pytest 收集但失败，结果为 `1 failed, 3 passed in 0.17s`；实际加速度为 `[0,3.35407184,0.59141336]`，期望为 `[0,1.72917699,0]`。
- 定位结果：测试把 `q_nb` 写成 `[cos(phi),sin(phi),0,0]`，实际表示 `2*phi` 的滚转；同时推力使用 `mg`，未按工况使用 `mg/cos(phi)`。此外近似断言应按修订计划设 `rtol=0`。
- 该失败及修正过程保留为 **H** 调试证据；下一步由本人改为半角四元数、补偿推力和规定容差后重跑。
